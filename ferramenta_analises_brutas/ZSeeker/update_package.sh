...
# Check if the Conda environment 'py310' exists using conda env list
env_exists=$(conda env list | grep -w py310)

# If environment doesn't exist, create it
if [ -z "$env_exists" ]; then
    echo "Conda environment 'py310' not found. Creating a new environment..."
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to create the Conda environment 'py310'. Exiting..."
        exit 1
    fi
fi
...
if [ $? -ne 0 ]; then
    echo "Failed to activate the Conda environment 'py310'. Exiting..."
    exit 1
fi
